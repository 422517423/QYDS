...
$(document).ready(function() {
	...
	//商品分类商品选择按钮的点击事件
	$('.icon-search-type').click(function() {
	...
	//品牌系列选择按钮的点击事件
	$('.icon-search-brand').click(function() {
		var attr = $('#selectGoodsBrandType').css('display');
		if(attr == 'none'){
			$('#selectGoodsBrandType').show();
		}else{
			$('#selectGoodsBrandType').hide();
		}
	});
	...
	$("#save_btn").click(function(){
		...
		json.goodsBrandTypeIdPath = $("#goods_brand_type_id_path").val();
		json.goodsBrandTypeId = $("#goods_brand_type_id").val();
		json.goodsBrandTypeCodePath = $("#goods_brand_type_code_path").val();
		json.goodsBrandTypeNamePath = $("#goods_brand_type_name_path").val();
		json.goodsBrandTypeCode = $("#goods_brand_type_code").val();
		...
		json.sizeDescription = $("#size_description").val();

		var jsonTextSpe = JSON.stringify(getSpecification());
		var jsonTextSpeSku = JSON.stringify(getSKUSpecification());
		//sku属性个数check 必须是两个以上
		var count = 0;
		var asciiC = "";
		var jsonsku = getSKUSpecification();
		for(var i = 0; i < jsonsku.length; i++ ){
			var ascii = jsonsku[i].specification_sku_ascii;
			if(asciiC != ascii){
				count = count + 1;
			}
			asciiC = ascii;
		}
		if($('#type').val() == '20' && count <= 1) {
		//if(count <= 1){
			showAlert("sku必须维护至少两个属性");
			return;
		}
		...
		init : function(){
			...
			function successFn() {
				...
				$('#confirmBrand').on('click',function(){
					var tree = $.fn.zTree.getZTreeObj("gdsBrandTypeTree");
					var selectNode = tree.getSelectedNodes()[0];
					...
					$('#goods_brand_type_id').val(selectNode.goodsTypeId);
					$('#goods_brand_type_code').val(selectNode.goodsTypeCode);

					//商品分类选择的层级关系
					var goods_brand_type_id_path = "";
					var goods_brand_type_code_path = "";
					var goods_brand_type_name_path = "";
					//组层级关系
					var goods_brand_type_id_path_array = new Array();
					var goods_brand_type_code_path_array = new Array();
					var goods_brand_type_name_path_array = new Array();

					while (selectNode != null){
						goods_brand_type_id_path_array.push(selectNode.goodsTypeId);
						goods_brand_type_code_path_array.push(selectNode.goodsTypeCode);
						goods_brand_type_name_path_array.push(selectNode.goodsTypeNameCn);
						selectNode = selectNode.getParentNode();
					}
					//把数组中存在的内容反转
					goods_brand_type_id_path_array.reverse();
					goods_brand_type_code_path_array.reverse();
					goods_brand_type_name_path_array.reverse();

					//再通过数组拼接字符串
					for(var i=0;i<goods_brand_type_id_path_array.length;i++){
						if(i==0){
							goods_brand_type_id_path = goods_brand_type_id_path_array[i];
						}else{
							goods_brand_type_id_path = goods_brand_type_id_path + "_" + goods_brand_type_id_path_array[i];
						}
					}
					for(var i=0;i<goods_brand_type_code_path_array.length;i++){
						if(i==0){
							goods_brand_type_code_path = goods_brand_type_code_path_array[i];
						}else{
							goods_brand_type_code_path = goods_brand_type_code_path + "_" + goods_brand_type_code_path_array[i];
						}
					}

					for(var i=0;i<goods_brand_type_name_path_array.length;i++){
						if(i==0){
							goods_brand_type_name_path = goods_brand_type_name_path_array[i];
						}else{
							goods_brand_type_name_path = goods_brand_type_name_path + "_" + goods_brand_type_name_path_array[i];
						}
					}
					//给画面端元素赋值
					$('#goods_brand_type_id_path').val(goods_brand_type_id_path);
					$('#goods_brand_type_code_path').val(goods_brand_type_code_path);
					$('#goods_brand_type_name_path').val(goods_brand_type_name_path);

					//隐藏分类的选择画面
					$('#selectGoodsBrandType').hide();

					if(goods_brand_type_name_path.length > 0){
...
function eidtSuccessFn(data){
	if (data.resultCode == '00') {
		...
		$('#size_description').val(item.size_description);
