...
$(document).ready(function() {
	...
	$("#btn_save").click(function(){
		...
		if(ids.length > 0){
			$("#suitlistEdit").modal('hide');
			sessionStorage.setItem("goodsIds",ids);
			oTZTable.fnDraw();
			//axse("/gds_suitlist/save.json", {'data':ids}, saveSuccessFn, errorFn);
		}else{
			showTip("没有选择商品组成套装");
		}
	});
...
//表格js初始化
function initTable() {
	oTable = $('#gds_master_table_suitlist').dataTable({
		"aoColumns": [
			{ "mData": null,
				"fnRender": function ( oObj ) {
					console.log(oObj.aData.checked);
		...
		"bServerSide": true,
		"bSort": false,
		"sAjaxSource": "../gds_suitlist/getList.json",
		"fnServerParams": function ( aoData ) {
			...
			aoData.push( { "name": "maintainStatus", "value": $("#maintain_status_suitlist").val() } );
		},
