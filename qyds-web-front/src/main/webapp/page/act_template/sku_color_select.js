...
$(document).ready(function () {
    $("#sku_color_select_dialog").modal('show');
    initTable();
    //检索按钮点击事件
    ...
    $("#sku_select_btn_confirm").click(function () {
        ...
        if (selectedSkuIds.length > 0) {
            $("#sku_color_select_dialog").modal('hide');
            // 这个方法在弹出选择模板的画面实现
            var selectedRows = [];
            $.each(selectedSkuIds, function (index, selectId) {
                $.each(tableSkuSelect.fnGetData(), function (index2, row) {
                    if (selectId == row.goodsCode + "_" + row.colorCode) {
                        selectedRows.push(row);
                    }
                });
            });
            onSkuColorSelected(selectedRows);
        } else {
            showTip("请先选择一个商品");
        }
    });
...
function initTable() {
    var tableOption = {
        ...
        "bFilter": false,
        "bSort": false,
        "sAjaxSource": "../gds_master/getSkuColorList.json",
        ...
        "aoColumns": [
            {
                ...
                "fnRender": function (rowData) {
                    var $radio = $('<input>')
                        ...
                        .attr('type', 'checkbox')
                        .attr('value', rowData.aData.goodsCode + "_" + rowData.aData.colorCode);
                    var $div = $('<div></div>');
                    $radio.appendTo($div);
                    return $div.html();
                }
