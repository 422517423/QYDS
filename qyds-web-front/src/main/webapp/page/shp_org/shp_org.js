        ...
        init : function(){
            ...
            function callback(data) {
                ...
                if(data.resultCode == '00'){
                    if($.isArray(data.data)){
                        ...
                        $.fn.zTree.init($("#orgTree"),settings, data.data);
                    }
                }else{
                    $('#alertMsg').show();
            ...
            function successFn() {
                ...
                $('#editBtn').on('click',function(){
                    $("#orgForm input,textarea,select").attr("disabled", true);
                    ...
                    $('#parentOrgDiv').hide();
                    $('#orgId').val(selectNode[0].orgId);
                    $('#orgidParent').val(selectNode[0].orgidParent);
                    $('#erpStoreId').val(selectNode[0].erpStoreId);
                    $('#orgType option[value="'+selectNode[0].orgType+'"]').attr('selected','selected');
                    if(selectNode[0].orgType == '20'){
                        $('input:text').attr("disabled","disabled");
                        $('#erpStoreId').closest('.form-group').show();
                    }else {
                        $('#erpStoreId').closest('.form-group').hide();
                    }
                    $('#orgCode').val(selectNode[0].orgCode);
                    $('#orgShortName').val(selectNode[0].orgShortName);
                    $('#orgName').val(selectNode[0].orgName);
                    $('#sort').val(selectNode[0].sort);
                    $('#comment').val(selectNode[0].comment);
                    $('#orgType').on('change', function() {
                        $("#orgForm").validate().resetForm();
                        var orgTypeVal = $('#orgType').val();
                        //门店的时候显示ERP门店ID
                        if(orgTypeVal == '20'){
                            $('#erpStoreId').closest('.form-group').show();
                        }else {
                ...
                $('#upBtn').on('click',function(){
                    ...
                    var data = new Array();
                    data.push({orgId: selectNode[0].orgId,sort: node.sort});
                    data.push({orgId: node.orgId,sort: selectNode[0].sort});
                    ajaxPost('/shp_org/resort.json',
                ...
                $('#downBtn').on('click',function(){
                    ...
                    data.push({orgId: selectNode[0].orgId,sort: node.sort});
                    data.push({orgId: node.orgId,sort: selectNode[0].sort});
                    ajaxPost('/shp_org/resort.json',{'data':JSON.stringify(data)},function (data) {
                        if($.type(data) == 'string' && data != ''){ƒ
                            data = $.parseJSON(data);
                        }
                        if(data.resultCode == '00'){
                ...
                $('#delBtn').on('click',function(){
                    var tree = $.fn.zTree.getZTreeObj("orgTree");
                    var selectNode = tree.getSelectedNodes();
                    showConfirm('确认删除该条店铺组织吗?',function(){
                        ajaxPost('/shp_org/delete.json',
                            {'orgId':selectNode[0].orgId,'sort':selectNode[0].sort,'orgidParent':selectNode[0].orgidParent},function(data){
                            if($.type(data) == 'string' && data != ''){
                                data = $.parseJSON(data);
                            }
                            if(data.resultCode == '00'){
                ...
                $('#saveBtn').on('click',function(){
                    ...
                    function callback(data){
                        ...
                        if(data.resultCode == '00'){
                            ...
                            if(data.flag == 'insert'){
                                var insertNode = {
                                    'orgId' : data.id,
                                    'orgidParent' : $('#orgidParent').val(),
                                    'orgType' : $('#orgType').val(),
                                    'orgCode' : $('#orgCode').val(),
                                    'erpStoreId' : $('#erpStoreId').val(),
                                    'orgShortName' : $('#orgShortName').val(),
                                    'sort' : sort,
                                    'orgName' : $('#orgName').val(),
                                    'comment' : $('#comment').val()
                                };
                                if($('#orgidParent').val() == '0'){
                                    tree.addNodes(null,'-1',insertNode,false);
                                }else{
                                    tree.addNodes(selectNode[0],'-1',insertNode,true);
                                }
                            }else{
                                selectNode[0].orgId = $('#orgId').val();
                                selectNode[0].orgidParent = $('#orgidParent').val();
                                selectNode[0].orgType = $('#orgType').val();
                                selectNode[0].orgCode = $('#orgCode').val();
                                selectNode[0].erpStoreId = $('#erpStoreId').val();
                                selectNode[0].orgShortName = $('#orgShortName').val();
                                selectNode[0].orgName = $('#orgName').val();
                                selectNode[0].comment = $('#comment').val();
                    ...
                    ajaxPost('/shp_org/save.json',form.serialize(),callback,null,null,null);
                    tree.cancelSelectedNode(selectNode[0]);
                    $('#button-groups a').not(':eq(0)').attr('disabled','disabled');
                });
            }
            //菜单树加载数据
            ajaxPost('/shp_org/getOrg.json', {}, callback, successFn, failFn, null);
