...
//退回到列表画面
function back() {
    $('#content').load('ord_master/list.html');
}
        ...
        "aoColumns": [
            {
                ...
                "fnRender": function (oObj) {
                    ...
                    // "退换货状态";"40";"待退款";
                    // "退换货状态";"41";"已退款";
                    if (oObj.aData.rexStatus == "10") {
                        strOperate += '<a href="javascript:void(0);" onclick=editDeliverStore("' + oObj.aData.rexOrderId + '");>&nbsp;修改退货门店&nbsp;</a>';
                        strOperate += '<a href="javascript:void(0);" onclick=returnGoodsApprove("' + oObj.aData.rexOrderId + '");>&nbsp;退货审批通过&nbsp;</a>';
                        strOperate += '<a href="javascript:void(0);" onclick=returnGoodsRejectConfirm("' + oObj.aData.rexOrderId + '");>&nbsp;退货审批驳回&nbsp;</a>';
                    }
                    if (oObj.aData.rexStatus == "20") {
                        strOperate += '<a href="javascript:void(0);" onclick=returnGoodsReceivedConfirm("' + oObj.aData.rexOrderId + '");>&nbsp;退货到货确认&nbsp;</a>';
                    }
 ...
 * @param obj
 */
function returnGoodsApprove(rexOrderId) {
    showConfirm("确定审批通过吗?", function () {
        var url = "/ord_return_goods/approve.json";
        var param = {
...
function editDeliverStore (rexOrderId){
    showConfirm("确定修改退货门店?", function () {
        sessionStorage.setItem("rexOrderId", rexOrderId);
        $('#customDialog').load('ord_return_goods/deliverStore.html');
    });
}
/**
 * 退货审批按钮
 ...
 * @param obj
 */
function returnGoodsRejectConfirm(rexOrderId) {
    $("#reject_dialog_approve_content").val("");
    rejectTempId = rexOrderId;
    $("#reject_dialog").modal('show');
}

function returnGoodsRejectSubmit() {
    var url = "/ord_return_goods/approve.json";
    var param = {
        "rexOrderId": rejectTempId,
        "rexStatus": "11",
        "applyAnswerComment": $("#reject_dialog_approve_content").val()
    };
    var success = function (data) {
        if (data.resultCode == '00') {
            showAlert("提交成功!");
 ...
 * @param obj
 */
function returnGoodsReceivedConfirm(rexOrderId) {
    showConfirm("确定收到货了吗?", function () {
        var url = "/ord_return_goods/receiveGoods.json";
        var param = {
            "rexOrderId": rexOrderId
        };
        var success = function (data) {
