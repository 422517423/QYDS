...
angular.module('test.controllers', [])
    ...
    // 登录 start
    .controller('loginCtrl', function ($scope, $state,$stateParams, authService,
        ...
        $scope.onLogin = function(){
            ...
            // 密码：123456
            // test data
            if('13988888888' == $scope.loginInfo.tel && '123456' == $scope.loginInfo.pd){
                // $state.go('purchase', {});// 我要采购
                // $state.go('purchaseInfo', {});// 我要供应
                // $state.go('myPurchase', {});//我的采购
                //$state.go('mySupply', {});//我的供应

                if($stateParams.type == '1'){
                    $state.go('purchase', {});
                }else if($stateParams.type == '2'){
                    $state.go('purchaseInfo', {});
                }else if($stateParams.type == '3'){
                    $state.go('myPurchase', {});
                }else {
                    $state.go('mySupply', {});
                }
                localStorageService.set(KEY_USERINFO, "logined");


            }else if('13999999999' == $scope.loginInfo.tel && '123456' == $scope.loginInfo.pd){
                popupService.showToast('账号正在审核中，请稍后尝试');
            }else{
                popupService.showToast('手机号或者密码输入错误');
            }

        };

        $scope.onResetPd = function(){
            popupService.showToast('迁移至重置密码页面');
        };
    ...
    .controller('purchaseInfoCtrl', function ($scope,$state, $rootScope,$ionicPopover, $ionicHistory,
                                          localStorageService,
                                          commonService,popupService, ionicDatePicker, $filter) {
        $rootScope.setTitle("采购信息");
        ...
        $scope.purchaseInfo = {tab:'0'};

        $scope.selectTab = function(index){
            $scope.purchaseInfo.tab = index;
        };
        $scope.onOffer = function(type){
            $state.go('offer', {type:type});
        };
        $scope.onDetail = function(type){
            $state.go('purchaseDetail', {type:type})
        };
    ...
    .controller('purchaseDetailCtrl', function ($scope,$state, $stateParams,$rootScope, $ionicHistory,
                                              localStorageService,
                                              commonService,popupService, ionicDatePicker, $filter) {
        $rootScope.setTitle("采购详细");

        $scope.purchaseInfo = {tab:$stateParams.type};
        if('0' == $stateParams.type){
            $scope.purchaseInfo.name = '动力煤';
        }else if('1' == $stateParams.type){
            $scope.purchaseInfo.name = '焦煤';
        }else if('2' == $stateParams.type){
            $scope.purchaseInfo.name = '焦炭';
        }

        $scope.selectTab = function(index){
            $scope.purchaseInfo.tab = index;
        };
        $scope.onOffer = function(){
            $state.go('offer', {})
        };

    })
    .controller('mySupplyCtrl', function ($scope,$state,$ionicPopover, $rootScope, $ionicHistory,
                                            localStorageService,
                                            commonService,popupService, ionicDatePicker, $filter) {

        $rootScope.setTitle("我的供应");
        $scope.mySupplyInfo = {tab:0};

        var userInfo = localStorageService.get(KEY_USERINFO);
        if (!userInfo) {
            $state.go("login", {"type": "4"});
            return;
        }

        $scope.selectTab = function(index){
            $scope.mySupplyInfo.tab = index;
        };

        $scope.onDetail = function(type){
            $state.go('mySupplyDetail', {"type":type,"status":$scope.mySupplyInfo.tab})
        };
    ...
    .controller('mySupplyDetailCtrl', function ($scope,$state,$stateParams, $rootScope, $ionicHistory,
                                                  localStorageService,
                                                  commonService,popupService, ionicDatePicker, $filter) {
        $rootScope.setTitle("供应详细");
        $scope.mySupplyDetailInfo={status:$stateParams.status,type:$stateParams.type};
        if('0' == $stateParams.type){
            $scope.mySupplyDetailInfo.name = '动力煤';
        }else if('1' == $stateParams.type){
            $scope.mySupplyDetailInfo.name = '焦煤';
        }else if('2' == $stateParams.type){
            $scope.mySupplyDetailInfo.name = '焦炭';
        }

    })
    .controller('myPurchaseCtrl', function ($scope,$state,$ionicPopover, $rootScope, $ionicHistory,
                                          localStorageService,
                                          commonService,popupService, ionicDatePicker, $filter) {

        $rootScope.setTitle("我的采购");
        $scope.myPurchaseInfo = {tab:0};

        var userInfo = localStorageService.get(KEY_USERINFO);
        if (!userInfo) {
            $state.go("login", {"type": "3"});
            return;
        }

        $scope.selectTab = function(index){
            $scope.myPurchaseInfo.tab = index;
        };

        $scope.onDetail = function(type){
            $state.go('myPurchaseDetail', {"type":type,"status":$scope.myPurchaseInfo.tab})
        };
        ...
        //        templateUrl: 'templates/sizeGuide.html',
    ...
    .controller('myPurchaseDetailCtrl', function ($scope,$state,$stateParams, $rootScope, $ionicHistory,
                                            localStorageService,
                                            commonService,popupService, ionicDatePicker, $filter) {
        $rootScope.setTitle("采购详细");
        $scope.myPurchaseDetailInfo={status:$stateParams.status,type:$stateParams.type};
        if('0' == $stateParams.type){
            $scope.myPurchaseDetailInfo.name = '动力煤';
        }else if('1' == $stateParams.type){
            $scope.myPurchaseDetailInfo.name = '焦煤';
        }else if('2' == $stateParams.type){
            $scope.myPurchaseDetailInfo.name = '焦炭';
        }

    })
    .controller('purchaseCtrl', function ($scope,$state, $rootScope, $ionicHistory,
                                          localStorageService,
                                          commonService,popupService, ionicDatePicker, $filter) {
        $rootScope.setTitle("我要采购");
        $scope.coalList = {
                '动力煤' : "0",
                '焦煤' : "1",
                '焦炭' : "2"
        ...
        $scope.openDatePicker = function (val) {
            var ipObj = {
                callback: function (val) {  //Mandatory
                    ...
                    $scope.purchaseInfo.endDate = $filter('date')(new Date(val),'yyyy-MM-dd');
                },
                disabledDates: [
                ],
                from: new Date(1950, 0, 1),
                to: new Date(),
                inputDate: $scope.purchaseInfo.endDate ? new Date($scope.purchaseInfo.endDate) : new Date("1990-01-01"),
                mondayFirst: true,
                closeOnSelect: true,
                templateType: 'popup',
    ...
    .controller('offerCtrl', function ($scope,$state,$stateParams, $rootScope, $ionicHistory,
                                            localStorageService,
                                            commonService,popupService, ionicDatePicker, $filter) {
        $rootScope.setTitle("我要报价");
        $scope.offerInfo={coalType:$stateParams.type};
        if('0' == $stateParams.type){
            $scope.offerInfo.name = '动力煤';
        }else if('1' == $stateParams.type){
            $scope.offerInfo.name = '焦煤';
        }else if('2' == $stateParams.type){
            $scope.offerInfo.name = '焦炭';
        }
    ...
    .controller('registerCtrl', function ($scope,$state, $rootScope, $ionicHistory,authService,
        ...
        // };
        //
        // $scope.registerInfo={tel:'',captcha:'',password:'',rePassword:'',agree:false, sex:'0',birthdate:''};
        //
        // // 日期选择器
        // $scope.openDatePicker = function (val) {
        //     var ipObj = {
    ...
    .controller('bagCtrl', function ($scope, $state,$rootScope,
                                     $ionicTabsDelegate, localStorageService, shoppingBagService,popupService) {
...
//门店限制取消
    ...
    .controller('goodsListCtrl', function ($scope, $state, $stateParams, $ionicPopover, $ionicTabsDelegate, localStorageService, goodsListService,popupService,$rootScope) {
        ...
        $scope.doGetAll = function($event){
            ...
            new goodsListService({"goodsTypeId": goodsTypeId, "actId":actId, "firstGoodsTypeId":firstGoodsTypeId,"memberId":memberId}).then(function (res) {
                if (res.resultCode == "00") {
                    $scope.goodsList = res.results;
                } else {
                    popupService.showToast(res.resultMessage);
        ...
        $scope.doResearch = function(isvalid){
            ...
            new goodsListService({"goodsTypeId": goodsTypeId, "actId":actId, "firstGoodsTypeId":firstGoodsTypeId,"from": from, "to": to,"memberId":memberId}).then(function (res) {
                $scope.isLoading = false;
                if (res.resultCode == "00") {
                    $scope.goodsList = res.results;
                } else {
    ...
    .controller('goodsListDisplayCtrl', function ($scope, $state, $stateParams, $ionicPopover, $ionicTabsDelegate, localStorageService, goodsListService,getNewDataService,goodsListDisplayService,popupService,$rootScope) {
        ...
        if(!$stateParams.title ){
                ...
                new getNewDataService({"firstGoodsTypeId": goodsTypeId})
                    .then(function (res) {
                        if (res.resultCode == "00") {
                            ...
                            //初始化调用
                            new goodsListService({"goodsIds": $scope.newDataList, "memberId":memberId,"cmsId":cmsId,"sortByTimeValue":$scope.sortByTimeValue}).then(function (res) {
                                if (res.resultCode == "00") {
                                    $scope.goodsList = res.results;
                                } else {
                                    popupService.showToast(res.resultMessage);
            ...
            //初始化调用
            new goodsListService({"goodsIds": goodsTypeId, "memberId":memberId,"cmsId":cmsId,"sortByTimeValue":$scope.sortByTimeValue}).then(function (res) {
                if (res.resultCode == "00") {
                    $scope.goodsList = res.results;
                } else {
                    popupService.showToast(res.resultMessage);
        ...
        $scope.doGetMore = function () {
            ...
            new goodsListService({"goodsIds": goodsTypeId, "memberId":memberId,"cmsId":cmsId,"sortByTimeValue":$scope.sortByTimeValue,"insertTimeGds":insertTime}).then(function (res) {
                if (res.resultCode == "00") {
                    if(res.results.length != 0){
                        $scope.goodsList = $scope.goodsList.concat(res.results);
                    }else {
    ...
    // 个人中心 end
    // 首页 start
    .controller('homePageCtrl', function ($scope, $state, $ionicTabsDelegate, localStorageService,
                                          indexFigureService, indexNewsService, indexNews1Service,indexNews2Service,indexNews3Service,indexArrivalsService, indexNewGoodsService,indexGdsBrandTypeService,
                                          indexRegion2Service,indexRegion3PhoneService,indexRegion4Service,indexRegion5Service,indexRegion6Service,indexRegion7Service,indexRegion8Service,
        ...
        $scope.goGoodsList2 = function(typeId,pageType,classifyId,classifyName){
            if(pageType != '3'){
                $state.go("goodsList", {"firstGoodsTypeId": typeId,"pageType":pageType,"classifyId":classifyId,"classifyName":classifyName});
            }else{
    ...
    // 首页 end
    // 分类 start
    .controller('classificationCtrl', function ($scope,$rootScope, $state, $ionicTabsDelegate, localStorageService, secondGdsTypeService, subGdsTypeService,secondGdsBrandTypeService,subGdsBrandTypeService, popupService) {
    ...
    // 分类 end
    // 全部优惠券 start
    .controller('couponAllCtrl', function ($scope,$rootScope, $state, $ionicTabsDelegate, localStorageService, couponAllService, getCouponService, popupService) {
    ...
    // 商品详情 start
    .controller('productDetailCtrl', function ($scope, $state, $stateParams, $ionicTabsDelegate,
                                               $ionicPopover, localStorageService, productDetailService,
                                               popupService, $sce, shoppingBagService, favoriteService,$rootScope) {
