...
public class SmsSendUtil {
    ...
    static public JSONObject sendPaymentNotice(MmbPointRecord mmbPointRecord, OrdMaster ordMaster, MmbMaster mmbMaster,SmsCaptchaConfig config) {
        // 初期化
        JSONObject result = new JSONObject();
        try {
            init(config);
            result = sendPaymentNotice(mmbPointRecord,ordMaster,mmbMaster);
        } catch (Exception e) {
            result.put("resultCode", Constants.FAIL);
            result.put("resultMessage", mmbMaster.getTelephone()+"消费通知短信发送失败");
        }
        return result;
    }

    static public JSONObject sendPaymentNotice(MmbPointRecord mmbPointRecord, OrdMaster ordMaster, MmbMaster mmbMaster) {
        JSONObject result = new JSONObject();
        try {
            // 初期化
            init();
            ...
            if(rsp.isSuccess()){
                ...
                result.put("resultMessage", mmbMaster.getTelephone()+"消费通知短信发送成功");
            }else {
                // 发送失败
                throw new Exception();
            }
