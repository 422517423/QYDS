    ...
    public JSONObject inputAll(String list){
        ...
        try{
            ...
            for (ErpStore record : ll) {
                ...
                store.setMchid(record.getMchid());
...
//            map.put("message", e.getMessage());
    ...
    @Override
    public ShpOrg selectByStoresubid(String storesubid) {
        List<ShpOrg> shpOrgs = mapperShpOrgExt.selectByStoresubid(storesubid);
        if (shpOrgs!=null&&shpOrgs.size()!=0){
           return shpOrgs.get(0);
        }
        return null;
    }
}
