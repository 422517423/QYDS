...
public class ActMasterServiceImpl implements ActMasterService {
    ...
    @Autowired
    MmbGroupMemberMapper mmbGroupMemberMapper;
    @Autowired
...
//        }
    ...
    private float getGoodsTotalPrice(List<OrdConfirmGoodsExt> validGoods) {
        ...
        for (int i = 0; i < validGoods.size(); i++) {
            ...
            if (!StringUtil.isEmpty(validGoods.get(i).getActGoodsId()) || (validGoods.get(i).getActivity().getNewPrice() != 0)) {
     ...
     * @return
     */
    public List<ViewOnsellSku> getNewPricesBySku(Map<String, SkuForm> skuFormMap, List<ViewOnsellSku> viewOnsellSkuList) {

        String goodsId = "", brandId = "", goodsTypeId = "", memberid = "";
        Map<String, Float> skuPriceMap = new ConcurrentHashMap<String, Float>();
        List<ViewOnsellSku> viewOnsellSkus = new ArrayList<ViewOnsellSku>();
        viewOnsellSkus = viewOnsellSkuList;
        for (String skuid : skuFormMap.keySet()) {
            skuPriceMap.put(skuid, skuFormMap.get(skuid).getPrice());
            if (!goodsId.equals(skuFormMap.get(skuid).getGoodsId())) {
                goodsId = skuFormMap.get(skuid).getGoodsId();
                brandId = skuFormMap.get(skuid).getBrandId();
                goodsTypeId = skuFormMap.get(skuid).getGoodsTypeId();
                memberid = skuFormMap.get(skuid).getMemberId();
            }
        }
        try {
            // 取出当前商品的所有活动
            Map<String, ActMasterForm> accordActMap = new ConcurrentHashMap<String, ActMasterForm>();
            List<ActMasterForm> activityList = actMasterMapperExt.selectAllActivitiesBySkuInfo
                    (goodsId, brandId, goodsTypeId, new ArrayList<String>(skuPriceMap.keySet()));
            if (activityList.size() > 0) {
                //判断当前账号状态(未登录，会员组，高级会员)能够参与的活动
                accordActMap = checkMemberActivity(activityList, memberid, goodsId);
            }
            if (accordActMap.size() > 0) {
                viewOnsellSkus = getActivityPrice(accordActMap, goodsId, skuPriceMap, viewOnsellSkuList);
            }
        ...
        for (String key : groupMap.keySet()) {
            ...
            if (goods.get(0).getActGoodsId() == null || goods.get(0).getActGoodsId().equals("")) {
                ...
                for (int i = 0; i < goods.size(); i++) {
                    ...
                    actMasterForm.setNewPrice(getDiscountPrice(orginPrice, memberDiscount * 10));
                ...
                ActTemplateForm actTemplateForm = new ActTemplateForm();
                actTemplateForm.setTempId(tempId);
                ActTemplateForm actTemplateForm1 = actTemplateMapperExt.selectById(actTemplateForm);
                ActTempParam actTempParam1 = new ActTempParam();
                actTempParam1.setTempId(tempId);
                List<ActTempParam> actTempParams = actTempParamMapperExt.selectByTempId(actTempParam1);
                String actitionType = actTemplateForm1.getActitionType();
                ...
                if (activity == null || !isContainsMember(activity, memberId)
                    ...
                    if (ComCode.ActivityType.DISCOUNT.equals(activity.getActivityType())) {
                        ...
                        for (int i = 0; i < goods.size(); i++) {
                            ...
                            if (actitionType.equals("20") && actTempParams.size() != 0) {
                                String paramValue = actTempParams.get(0).getParamValue();
                                double b = Double.parseDouble(paramValue);
                                //添加判断值
//                              if (!(activity.getTempId()).equals("13970196-c36d-4b49-88ba-824d98b0a0dc")){
                        ...
                        for (int i = 0; i < goods2.size() / 2; i++) {
                            ...
                            //新乘会员折扣
                            int count = mmbGoodsMapper.selectBrandExistByGoodsId(goods2.get(i).getGoodsId());
                            if (count != 0 && ((discount1 * 10) / 100) > 0.57) {
...
//                           //新乘会员折扣
                            int count2 = mmbGoodsMapper.selectBrandExistByGoodsId(goods2.get(goods2.size() - i - 1).getGoodsId());
                            if (count2 != 0 && ((discount2 * 10) / 100) > 0.57) {
...
//                            }
                        ...
                        if (goods2.size() % 2 != 0) {
                            ...
                            //乘会员折扣
                            int count = mmbGoodsMapper.selectBrandExistByGoodsId(goods.get(middleIndex).getGoodsId());
                            if (count != 0) {
                                newPrice3 = getDiscountPrice(newPrice3, memberDiscount * 10);
                                if (memberDiscount < 1) {
                        ...
                        for (int i = 0; i < goods.size(); i++) {
                            ...
                            // 乘会员折扣
                            int count = mmbGoodsMapper.selectBrandExistByGoodsId(goods.get(i).getGoodsId());
                            if (count != 0 && newPrice / orginPrice > 0.57) {
                                newPrice = getDiscountPrice(newPrice, memberDiscount * 10);
                                if (memberDiscount < 1) {
                                    goods.get(i).setFlag(true);
                                }
     ...
     * @param activityList
     */
    private Map<String, ActMasterForm> checkMemberActivity(List<ActMasterForm> activityList, String memberId, String goodsId) {

//        List<ActMasterForm> accordActivities = new ArrayList<ActMasterForm>();
        Map<String, ActMasterForm> mmbActMap = new ConcurrentHashMap<String, ActMasterForm>();
        Map<String, ActMasterForm> allActMap = new ConcurrentHashMap<String, ActMasterForm>();
        Map<String, ActMasterForm> accordActMap = new ConcurrentHashMap<String, ActMasterForm>();
        for (int j = 0; j < activityList.size(); j++) {
            String memberType = activityList.get(j).getMemberType();
            //判断是否是秒杀 true为秒杀活动时间正确和非秒杀活动,false为秒杀活动但是时间不正确
            if (!checkSecKillActivity(activityList.get(j))) {
                continue;
            }
            if (!ComCode.ActivityMemberType.ALL.equals(memberType)) {
                mmbActMap.put(activityList.get(j).getActivityId(), activityList.get(j));
                continue;
            }
            allActMap.put(activityList.get(j).getActivityId(), activityList.get(j));
        }
...
//        accordActivities.addAll(allActMap.values());
        ...
        if (mmbActMap.keySet().size() > 0) {
            //活动对应的信息
            List<ActMember> actMembers = actMemberMapper.selectByActivityIds(new ArrayList<String>(mmbActMap.keySet()));


            for (String activityid : mmbActMap.keySet()) {
                ActMasterForm actMasterForm = mmbActMap.get(activityid);
                String memberType = actMasterForm.getMemberType();
                switch (memberType) {
                    //指定会员组活动
                    case ComCode.ActivityMemberType.MEMBER_GROUP:
                        boolean isAccord = false;
                        for (MmbGroupMember mmbGroupMember : mmbGroupMembers) {
                            for (ActMember actMember : actMembers) {
                                if (mmbGroupMember.getGroupId().equals(actMember.getMemberId())) {
                                    isAccord = true;
                        ...
                        if (isAccord) {
                            accordActMap.put(actMasterForm.getActivityId(), actMasterForm);
                        }
                        break;
                    //高级会员活动
                    case ComCode.ActivityMemberType.MEMBER_LEVEL:
                        for (ActMember actMember : actMembers) {
                            if (member.getMemberLevelId().equals(actMember.getMemberId())) {
                                accordActMap.put(actMasterForm.getActivityId(), actMasterForm);
                                continue;
                            }
                        }
    ...
    private List<ViewOnsellSku> getActivityPrice(Map<String, ActMasterForm> actMasterForms, String goodsId, Map<String, Float> skuPriceMap, List<ViewOnsellSku> viewOnsellSkuList) {

        Map<String, String> activityTemp = new ConcurrentHashMap<String, String>();
        Map<String, List<ActTempParam>> tempParam = new ConcurrentHashMap<String, List<ActTempParam>>();
        Map<String, List<ActGoods>> activityGoods = new ConcurrentHashMap<String, List<ActGoods>>();
        Map<String, ViewOnsellSku> viewOnsellSkuMap = new ConcurrentHashMap<String, ViewOnsellSku>();

        for (ViewOnsellSku viewOnsellSku : viewOnsellSkuList) {
            viewOnsellSkuMap.put(viewOnsellSku.getSkuid(), viewOnsellSku);
        }

        List<ActMaster> actMasters = actMasterMapper.selectByActivityIds(new ArrayList<String>(actMasterForms.keySet()));
        for (ActMaster actMaster : actMasters) {
            activityTemp.put(actMaster.getActivityId(), actMaster.getTempId());
        }

        List<ActTempParam> actTempParams = actTempParamMapperExt.selectByTempIds(new ArrayList<String>(activityTemp.values()));
        for (ActTempParam actTempParam : actTempParams) {
            if (tempParam.get(actTempParam.getTempId()) == null) {
                tempParam.put(actTempParam.getTempId(), new ArrayList<ActTempParam>(Arrays.asList(actTempParam)));
            } else {
                tempParam.get(actTempParam.getTempId()).add(actTempParam);
            }
        }

        List<ActGoods> actGoodsList = actGoodsMapper.selectByGoodsActivityIds(new ArrayList<String>(actMasterForms.keySet()), goodsId);
        for (ActGoods actGoods : actGoodsList) {
            if (activityGoods.get(actGoods.getActivityId()) == null) {
                activityGoods.put(actGoods.getActivityId(), new ArrayList<ActGoods>(Arrays.asList(actGoods)));
            } else {
                activityGoods.get(actGoods.getActivityId()).add(actGoods);
            }
        }
        for (String skuid : skuPriceMap.keySet()) {
                BigDecimal oldPrice = new BigDecimal(String.valueOf(skuPriceMap.get(skuid)));
                BigDecimal newPrice = new BigDecimal(String.valueOf(skuPriceMap.get(skuid)));
                viewOnsellSkuMap.get(skuid).setPrice(skuPriceMap.get(skuid).toString());
                viewOnsellSkuMap.get(skuid).setActivityPrice(skuPriceMap.get(skuid).toString());

            for (String activityId : actMasterForms.keySet()) {

                ActMasterForm actMasterForm = actMasterForms.get(activityId);
                String actitionType = actMasterForm.getActivityType();
                String goodsType = actMasterForm.getGoodsType();
                boolean isAccord = false;
                if (ComCode.ActivityGoodsType.SKU.equals(goodsType)) {

                    for (ActGoods actGoods : activityGoods.get(activityId)) {
                        if (skuid.equals(actGoods.getSkuId())) {
                            isAccord = true;
                        }
                    }
                    if (!isAccord) {
                ...
                switch (actitionType) {
                    // "10";"单品特价";
                    case ComCode.ActivityType.SPECIAL_PRICE:

                        newPrice = new BigDecimal(tempParam.get(activityTemp.get(activityId)).get(0).getParamValue());
                        ...
                        break;
                    // "11"："秒杀";
                    case ComCode.ActivityType.SECOND_KILL:

                        for (ActGoods actGoods : activityGoods.get(activityId)) {
                            if (skuid.equals(actGoods.getSkuId())) {
                                setActivityPrice(viewOnsellSkuMap.get(skuid),
                                        Float.valueOf(actGoods.getActPrice().toString()),
                                        actMasterForms.get(activityId).getActivityType(), actMasterForms.get(activityId).getActivityName());
                            }
                        }
                        break;
                    // "20";"折扣";
                    case ComCode.ActivityType.DISCOUNT:

                        for (ActTempParam actTempParam : tempParam.get(activityTemp.get(activityId))) {
                            if (actTempParam.getParamCondition().equals("1")) {
                                newPrice = oldPrice.multiply
                                        (new BigDecimal(actTempParam.getParamValue())).
                        ...
                        break;
                    //"21";"一二件折";
                    case ComCode.ActivityType.DISCOUNT_TWO:
                        ...
                        break;
                    //"30";"积分换购";
                    case ComCode.ActivityType.INTEGRAL_EXCHANGE:
                        newPrice = new BigDecimal(tempParam.get(activityTemp.get(activityId)).get(0).getParamValue());
                        setActivityPrice(viewOnsellSkuMap.get(skuid),
                                Float.valueOf(newPrice.toString()),
                                actMasterForms.get(activityId).getActivityType(), actMasterForms.get(activityId).getActivityName());
                        break;
                    //"40";"订单满送优惠劵";
                    case ComCode.ActivityType.FULL_SEND:
                        //do nothing
                        break;
                    //"41";"订单满减";
                    case ComCode.ActivityType.FULL_CUT:
                        ...
                        if (oldPrice.compareTo(new BigDecimal(tempParam.get(activityTemp.get(activityId)).get(0).getParamCondition())) >= 0) {
                            newPrice = newPrice.subtract
                                    (new BigDecimal(tempParam.get(activityTemp.get(activityId)).get(0).getParamValue()));
                            setActivityPrice(viewOnsellSkuMap.get(skuid),
                                    Float.valueOf(newPrice.toString()),
                                    actMasterForms.get(activityId).getActivityType(), actMasterForms.get(activityId).getActivityName());
                        }
                    ...
                    case ComCode.ActivityType.FULL_SEND_GOODS:
                        ...
                        break;
                    //"43";"订单满送积分";
                    case ComCode.ActivityType.FULL_SEND_POINT:
                        //do nothing
                        break;
                    //"44";"订单满折"
                    case ComCode.ActivityType.FULL_DISCOUNT:
                        if (oldPrice.compareTo(new BigDecimal(tempParam.get(activityTemp.get(activityId)).get(0).getParamCondition())) >= 0) {
                            newPrice = oldPrice.multiply
                                    (new BigDecimal(tempParam.get(activityTemp.get(activityId)).get(0).getParamValue())).
                                    divide(new BigDecimal("10"), 2, RoundingMode.HALF_UP);
        ...
        return new ArrayList<ViewOnsellSku>(viewOnsellSkuMap.values());
    }

    private void setActivityPrice(ViewOnsellSku viewOnsellSku, Float activityPrice, String activityType, String activityName) {
        if (activityPrice.compareTo(Float.valueOf(viewOnsellSku.getActivityPrice())) < 0) {
            viewOnsellSku.setActivityPrice(activityPrice.toString());
            viewOnsellSku.setActivityType(activityType);
            viewOnsellSku.setActivityName(activityName);
        }
    }
