package net.dlyt.qyds.web.controller.order_pc;
...
public class OrderPcController {
    ...
    @ResponseBody
    public JSONObject getOrderList(String data){
        ...
        try{
            OrdWechatForm form = JSON.parseObject(data, OrdWechatForm.class);
            //获取查询条件数据
            ...
            if(list == null || list.size() == 0){
                ...
                for (OrdMasterExt ome : list) {
                    ...
                    jsonObject.put("deliverStatus", ome.getDeliverStatus());
                    ...
                    jsonObject.put("couponId", ome.getCouponMemberId());
                ...
                int tPage = dataJson.getIntValue("totalPage");
                if(arrayMaster.size() > 0 && tPage == 0){
                    if(ordCounts%arrayMaster.size() > 0){
                        tPage = (ordCounts/arrayMaster.size()) + 1;
                    }else{
                        tPage = (ordCounts/arrayMaster.size());
                    }
                }

                String type = dataJson.getString("type");
                if(type.equals("1")){
                    json.put("totalCount", ordCounts);
                }else if(type.equals("2")){
                    json.put("totalCount", notCount);
                }else if(type.equals("3")){
                    json.put("totalCount", notDeliverCount);
                }else if(type.equals("4")){
                    json.put("totalCount", receiveCount);
                }
                json.put("aaData", arrayMaster);
                json.put("totalPage", tPage);
                json.put("resultCode", Constants.NORMAL);
            }
    ...
    JSONObject getOrderDetail(String data) {
        ...
        try {
            ...
            if(list == null || list.size() == 0){
                ...
                jsonMaster.put("payTime",list.get(0).getPayTime());
...
//                jsonConfirm.put("goodsInfo",arrayGoodsInfo);
    ...
    JSONObject submitOrder(String data){
        ...
        try {
            ...
            if(result == null){
                ...
                json.put("resultsOrderId", result.getOrderId());
                //开启定时任务
    ...
    JSONObject queryLogisticsInfo(String data){
        ...
        try {
            ...
            if(StringUtil.isEmpty(form.getOrderId())){
                throw new ExceptionErrorParam("缺少主键订单ID");
            }

           json = ordMasterService.queryLogisticsInfo(form.getOrderId());
     ...
     * @return
     */
    @RequestMapping("checkInputTel")
    public @ResponseBody
    JSONObject checkInputTel(String data){
        JSONObject jsonObject = JSON.parseObject(data);
        String tel = (String) jsonObject.get("tel");
        JSONObject json = new JSONObject();
        try {
            json = ordMasterService.checkInputTel(tel);
        }catch(Exception e){
            json.put("resultCode", Constants.FAIL);
            json.put("message", e.getMessage());
        }
